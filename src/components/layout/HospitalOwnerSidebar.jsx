...
import { HOSPITAL_OWNER_NAV } from '../../config/navigation';
import { LogOut, ChevronDown, ChevronRight } from 'lucide-react';

const HospitalOwnerSidebar = ({ isCollapsed, isMobileOpen, setIsMobileOpen }) => {
              ...
              return (
                <div key={item.name} className="flex flex-col gap-1">
                  <button
                    ...
                    className={`flex items-center justify-between w-full py-2.5 rounded-lg transition-all duration-200 overflow-hidden px-4 gap-3 ${isCollapsed ? 'md:justify-center md:px-0' : ''} ${
                      isChildActive && !isOpen ? 'bg-primary-50 text-primary-600 font-medium' : 'text-secondary-600 hover:bg-secondary-50 hover:text-secondary-900'
                    }`}
                  >
                    ...
                    {!isCollapsed && (
                      isOpen ? <ChevronDown className="w-4 h-4 shrink-0 text-secondary-400" /> : <ChevronRight className="w-4 h-4 shrink-0 text-secondary-400" />
                    )}
                  </button>
                  
                  {isOpen && !isCollapsed && (
                    <div className="flex flex-col gap-1 pl-11 pr-2 mt-1 mb-1">
                      {item.children.map((child) => {
                        ...
                        return (
                          <NavLink
                            ...
                            onClick={() => setIsMobileOpen(false)}
                            className={({ isActive }) =>
                              `flex items-center py-2 rounded-lg transition-all duration-200 gap-3 px-3 ${isActive
                                ? 'bg-primary-500 text-white font-medium shadow-sm shadow-primary-500/10'
                                : 'text-secondary-500 hover:text-primary-600 hover:bg-primary-50/50'
                              }`
                            }
                          >
                            <ChildIcon className="shrink-0 w-[14px] h-[14px]" />
...
export default HospitalOwnerSidebar;
