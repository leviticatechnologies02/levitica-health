...
const Laboratory = () => {
  ...
  const handleSave = (e) => {
    ...
    if (selectedRecord) {
      ...
      setData([{ ...formData, id: `LAB-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false }, ...data]);
    }
    setIsFormOpen(false);
  };
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
        <h3 className="text-indigo-800 font-semibold mb-1">Module Note: Laboratory Module</h3>
        <p className="text-indigo-600 text-sm"><strong>Flow/Usefulness:</strong> Digitizing lab workflows ensures that diagnostic reports are instantly attached to patient profiles. This eliminates paper trails, speeds up diagnoses, and alerts doctors immediately when critical results are ready.</p>
      </div>

      {/* HEADER & ACTIONS */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Laboratory</h1>
          <p className="text-slate-500 text-sm mt-1">Manage laboratory tests, categories, and pricing.</p>
        </div>
