...
const Laboratory = () => {
  ...
  const stats = [
    { title: 'Total Orders', value: '4', icon: ClipboardList, colorTheme: 'blue', trendLabel: 'vs yesterday' },
    { title: 'Pending Results', value: '2', icon: Clock, colorTheme: 'orange', trendLabel: 'vs yesterday' },
    { title: 'Critical Alerts', value: '1', icon: AlertCircle, colorTheme: 'rose', trendLabel: 'vs yesterday' },
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    if (selectedRecord) {
      setCatalogData(catalogData.map(item => item.id === selectedRecord.id ? { ...values, id: item.id, isDeleted: item.isDeleted } : item));
    } else {
      setCatalogData([{ ...values, id: `LAB-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false }, ...catalogData]);
    }
    setIsFormOpen(false);
    setSubmitting(false);
  };
  ...
  const catalogColumns = [
    ...
    { header: 'Turnaround (TAT)', accessor: 'tat' },
    {
      ...
      accessor: 'status',
      render: (item) => (
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${item.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-600 border-slate-200'
          }`}>
    ...
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="flex border-b border-slate-200 mb-6">
        <button
          ...
          className={`pb-3 px-4 text-sm font-medium border-b-2 transition-colors ${activeTab === 'orders' ? 'border-primary-600 text-primary-600' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
        >
          Lab Orders
        </button>
        <button
          ...
          className={`pb-3 px-4 text-sm font-medium border-b-2 transition-colors ${activeTab === 'catalog' ? 'border-primary-600 text-primary-600' : 'border-transparent text-slate-500 hover:text-slate-700'
            }`}
        >
          Test Catalog
        </button>
      ...
      <Modal
        ...
        title={selectedRecord ? 'Edit Test' : 'Add Test'}
      >
        <Formik
          initialValues={selectedRecord || { testName: '', category: '', price: '', tat: '', status: 'Active' }}
            ...
            <Form className="p-6">
              <div className="space-y-4">
                <div>
                  ...
                  <Field
                    name="testName"
                    type="text"
                    className={`w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none ${errors.testName && touched.testName ? 'border-red-500' : 'border-slate-200'}`}
                  />
                  <ErrorMessage name="testName" component="div" className="text-red-500 text-xs mt-1" />
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Field
                      name="tat"
                      type="text"
                      className={`w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none ${errors.tat && touched.tat ? 'border-red-500' : 'border-slate-200'}`}
                    />
                    <ErrorMessage name="tat" component="div" className="text-red-500 text-xs mt-1" />
                  </div>
                </div>
                <div>
              ...
              <div className="mt-8 flex justify-end gap-3">
                ...
                <button type="submit" disabled={isSubmitting} className="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 shadow-sm shadow-primary-600/20 transition-colors disabled:opacity-70">
                  {isSubmitting ? 'Saving...' : (selectedRecord ? 'Update' : 'Add Test')}
                </button>
              </div>
            </Form>
          )}
