...
const MOCK_DATA = [
  { id: 'RAD-001', scanType: 'MRI Brain', machine: 'MRI Scanner A', cost: '$450.00', status: 'Active', isDeleted: false },
  { id: 'RAD-002', scanType: 'CT Chest', machine: 'CT Scanner 1', cost: '$300.00', status: 'Active', isDeleted: false },
  { id: 'RAD-003', scanType: 'X-Ray Chest PA', machine: 'X-Ray Room B', cost: '$50.00', status: 'Inactive', isDeleted: false },
];
...
const Radiology = () => {
  ...
  const [sortConfig, setSortConfig] = useState({ key: 'scanType', direction: 'asc' });

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [selectedRecord, setSelectedRecord] = useState(null);
  
  const initialForm = { scanType: '', machine: '', cost: '', status: 'Active' };
  ...
  const handleSave = (e) => {
    ...
    if (selectedRecord) {
      ...
      setData([{ ...formData, id: `RAD-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false }, ...data]);
    }
    setIsFormOpen(false);
  };
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
        <h3 className="text-indigo-800 font-semibold mb-1">Module Note: Radiology Module</h3>
        <p className="text-indigo-600 text-sm"><strong>Flow/Usefulness:</strong> Radiology integration allows for secure storage and sharing of large medical images (DICOM). It enables remote consultations and ensures imaging results are immediately available in the doctor's consultation room.</p>
      </div>

      {/* HEADER & ACTIONS */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Radiology</h1>
          <p className="text-slate-500 text-sm mt-1">Manage radiology scans, machines, and pricing.</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={openCreate} className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium shadow-sm shadow-primary-600/20">
            <Plus className="w-4 h-4" /> Add Scan Type
          </button>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500 font-semibold">
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('id')}>Scan ID</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('scanType')}>Scan Type</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('machine')}>Machine</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('cost')}>Cost</th>
              ...
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-slate-900">{item.scanType}</span>
                  </div>
                </td>
                <td className="px-6 py-4 text-sm text-slate-600">{item.machine}</td>
                <td className="px-6 py-4 text-sm text-slate-900 font-medium">{item.cost}</td>
                <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-lg font-bold text-slate-900">{selectedRecord ? 'Edit Scan' : 'Add Scan Type'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6">
              <div className="space-y-4">
                <div>
                  ...
                  <input required type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" value={formData.scanType} onChange={e => setFormData({...formData, scanType: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Machine</label>
                  <input required type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" value={formData.machine} onChange={e => setFormData({...formData, machine: e.target.value})} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Cost</label>
                    <input required type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" value={formData.cost} onChange={e => setFormData({...formData, cost: e.target.value})} />
                  </div>
                  <div>
              ...
              <div className="mt-8 flex justify-end gap-3">
                ...
                <button type="submit" className="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 shadow-sm shadow-primary-600/20 transition-colors">
                  {selectedRecord ? 'Update' : 'Add Scan'}
                </button>
              </div>
            </form>
          </div>
