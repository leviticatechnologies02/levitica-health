...
import { 
  Search, Filter, Plus, Edit2, Trash2, Bed
} from 'lucide-react';

const MOCK_DATA = [
  { id: 'BED-001', number: 'B-01', room: '101', type: 'Standard', status: 'Available', isDeleted: false, createdAt: '2023-01-15' },
  { id: 'BED-002', number: 'B-02', room: '102', type: 'ICU Bed', status: 'Occupied', isDeleted: false, createdAt: '2023-02-20' },
];
...
const Beds = () => {
  ...
  const initialForm = { number: '', type: '', room: '', status: 'Available' };
  ...
  const handleSave = (e) => {
    ...
    if (selectedRecord) {
      ...
      setData([{ ...formData, id: `BED-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
  };
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
        <h3 className="text-indigo-800 font-semibold mb-1">Module Note: Track individual bed status, occupancy, and maintenance schedules.</h3>
        <p className="text-indigo-600 text-sm"><strong>Flow/Usefulness:</strong> Granular bed management is crucial for real-time admission decisions. It prevents overbooking and allows staff to instantly see which beds are occupied, being cleaned, or available for new incoming patients.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Track individual bed status, occupancy, and maintenance schedules.</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500 font-semibold">
              ...
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('number')}>Bed Number</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('type')}>Type</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('room')}>Room</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {paginatedData.length > 0 ? paginatedData.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-4 text-sm text-slate-600">{item.room}</td>
                <td className="px-6 py-4">
                  <button 
            ...
            <form onSubmit={handleSave} className="p-6">
              <div className="space-y-4">
                <div>
                  ...
                  <input required type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" value={formData.number} onChange={e => setFormData({...formData, number: e.target.value})} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input required type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" value={formData.room} onChange={e => setFormData({...formData, room: e.target.value})} />
                </div>
                <div>
