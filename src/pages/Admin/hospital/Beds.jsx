...
const BedSchema = Yup.object().shape({
  number: Yup.string().required('Bed Number is required'),
...
const Beds = () => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    if (selectedRecord) {
      ...
      setData([{ ...values, id: `BED-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
    setSubmitting(false);
  };
  ...
  const columns = [
    ...
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('number')}>Bed Number</div>,
      accessor: 'number',
      render: (item) => (
    ...
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('room')}>Room</div>,
      accessor: 'room',
      render: (item) => <span className="text-sm text-slate-600">{item.room}</span>
    },
    {
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      >
        <Formik
          initialValues={selectedRecord || { number: '', type: '', room: '', status: 'Available' }}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Field
                    ...
                    type="text"
                    placeholder="e.g. 101"
                    className={`w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none ${errors.room && touched.room ? 'border-red-500' : 'border-slate-200'}`}
                  />
                  <ErrorMessage name="room" component="div" className="text-red-500 text-xs mt-1" />
                </div>
                <div>
