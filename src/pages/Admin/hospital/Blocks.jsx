...
const Blocks = () => {
  ...
  const initialForm = { name: '', type: '', floors: '', status: 'Active' };
  ...
  const handleSave = (e) => {
    ...
    if (selectedRecord) {
      ...
      setData([{ ...formData, id: `BLK-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
  };
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
        <h3 className="text-indigo-800 font-semibold mb-1">Module Note: Manage large infrastructure blocks and buildings within the hospital campus.</h3>
        <p className="text-indigo-600 text-sm"><strong>Flow/Usefulness:</strong> For multi-building hospital campuses, mapping out 'Blocks' (e.g., North Block, OPD Block) is crucial for accurate patient navigation, emergency evacuation planning, and asset tracking per building.</p>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500 font-semibold">
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('id')}>Block ID</th>
              ...
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('floors')}>Floors</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {paginatedData.length > 0 ? paginatedData.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-4 text-sm text-slate-600">{item.floors}</td>
                <td className="px-6 py-4">
                  <button 
            ...
            <form onSubmit={handleSave} className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input required type="number" className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" value={formData.floors} onChange={e => setFormData({...formData, floors: e.target.value})} />
                </div>
                <div>
