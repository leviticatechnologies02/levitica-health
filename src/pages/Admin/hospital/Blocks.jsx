...
import {
  Search, Filter, Plus, Edit2, Trash2, Blocks as BlocksIcon,
...
const MOCK_DATA = [
  { id: 'BLK-001', name: 'North Block', type: 'Main Hospital', floors: 5, status: 'Active', isDeleted: false, createdAt: '2023-01-15' },
  { id: 'BLK-002', name: 'South Block', type: 'Outpatient (OPD)', floors: 3, status: 'Active', isDeleted: false, createdAt: '2023-02-20' },
  { id: 'BLK-003', name: 'East Block', type: 'Emergency', floors: 2, status: 'Active', isDeleted: false, createdAt: '2023-03-10' },
  { id: 'BLK-004', name: 'West Block', type: 'Intensive Care Unit (ICU)', floors: 4, status: 'Inactive', isDeleted: false, createdAt: '2023-04-05' },
];

const BlockSchema = Yup.object().shape({
  name: Yup.string().required('Block Name is required'),
  type: Yup.string().required('Block Type is required'),
  floors: Yup.number().min(1, 'Must have at least 1 floor').required('Total Floors is required'),
...
const Blocks = () => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    if (selectedRecord) {
      ...
      setData([{ ...values, id: `BLK-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
    setSubmitting(false);
  };
  ...
  const columns = [
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('id')}>Block ID</div>,
      accessor: 'id',
      render: (item) => <span className="font-medium text-slate-900">{item.id}</span>
    },
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('name')}>Name</div>,
      accessor: 'name',
      render: (item) => (
    ...
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('floors')}>Floors</div>,
      accessor: 'floors',
      render: (item) => <span className="text-sm text-slate-600">{item.floors}</span>
    },
    {
      ...
      render: (item) => (
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${item.status === 'Active' || item.status === 'Available' ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
            }`}
        >
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Hospital Blocks</h1>
          <p className="text-slate-500 text-sm mt-1">Manage large infrastructure blocks and buildings.</p>
        </div>
      ...
      >
        <Formik
          initialValues={selectedRecord || { name: '', type: '', floors: '', status: 'Active' }}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Field
                    name="type"
                    type="text"
                    placeholder="e.g. Outpatient (OPD)"
                ...
                <div>
                  ...
                  <Field
                    ...
                    type="number"
                    placeholder="e.g. 5"
                    className={`w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none ${errors.floors && touched.floors ? 'border-red-500' : 'border-slate-200'}`}
                  />
                  <ErrorMessage name="floors" component="div" className="text-red-500 text-xs mt-1" />
                </div>
                <div>
