...
const Floors = () => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    if (selectedRecord) {
      ...
      setData([{ ...values, id: `FLR-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
    setSubmitting(false);
  };
  ...
  const columns = [
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('id')}>Floor ID</div>,
      accessor: 'id',
      render: (item) => <span className="font-medium text-slate-900">{item.id}</span>
    },
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('name')}>Floor Name</div>,
      accessor: 'name',
      render: (item) => (
    ...
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('block')}>Block</div>,
      accessor: 'block',
      render: (item) => <span className="text-sm text-slate-600">{item.block}</span>
    },
    {
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      >
        <Formik
          initialValues={selectedRecord || { name: '', block: '', capacity: '', status: 'Active' }}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Field
                    ...
                    className={`w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none ${errors.block && touched.block ? 'border-red-500' : 'border-slate-200'}`}
                  />
                  <ErrorMessage name="block" component="div" className="text-red-500 text-xs mt-1" />
                </div>
                <div>
