...
import { 
  Search, Filter, Plus, Edit2, Trash2, Layers
} from 'lucide-react';

const MOCK_DATA = [
  { id: 'FLR-001', name: 'Ground Floor', block: 'North Block', capacity: 50, status: 'Active', isDeleted: false, createdAt: '2023-01-15' },
  { id: 'FLR-002', name: 'First Floor', block: 'North Block', capacity: 100, status: 'Active', isDeleted: false, createdAt: '2023-02-20' },
];
...
const Floors = () => {
  ...
  const initialForm = { name: '', block: '', capacity: '', status: 'Active' };
  ...
  const handleSave = (e) => {
    ...
    if (selectedRecord) {
      ...
      setData([{ ...formData, id: `FLR-00${Math.floor(Math.random() * 90) + 10}`, isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
  };
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
        <h3 className="text-indigo-800 font-semibold mb-1">Module Note: Manage floor plans and level configurations across hospital blocks.</h3>
        <p className="text-indigo-600 text-sm"><strong>Flow/Usefulness:</strong> Categorizing infrastructure by floor helps in efficiently assigning nursing stations to specific levels. It also aids maintenance staff and security teams in localizing issues to exact levels within a block.</p>
      </div>

      {/* HEADER & ACTIONS */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Hospital Floors</h1>
          <p className="text-slate-500 text-sm mt-1">Manage floor plans and level configurations across hospital blocks.</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500 font-semibold">
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('id')}>Floor ID</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('name')}>Floor Name</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('block')}>Block</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {paginatedData.length > 0 ? paginatedData.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-4 text-sm text-slate-600">{item.block}</td>
            ...
            <form onSubmit={handleSave} className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input required type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" value={formData.block} onChange={e => setFormData({...formData, block: e.target.value})} />
                </div>
                <div>
