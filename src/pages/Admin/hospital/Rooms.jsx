...
const Rooms = () => {
  ...
  const initialForm = { number: '', type: '', ward: '', price: '', status: 'Available' };
  ...
  const handleSave = (e) => {
    ...
    if (selectedRecord) {
      ...
      setData([{ ...formData, id: `RM-${Math.floor(Math.random() * 900) + 100}`, isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
  };
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
        <h3 className="text-indigo-800 font-semibold mb-1">Module Note: Manage room allocations, types, and pricing configurations.</h3>
        <p className="text-indigo-600 text-sm"><strong>Flow/Usefulness:</strong> Tracking rooms individually allows the hospital to manage different room types (e.g., Private, Semi-Private, General) and auto-calculate room tariffs during the billing process, ensuring accurate patient charging.</p>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-slate-500 font-semibold">
              ...
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('number')}>Room Number</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('type')}>Type</th>
              <th className="px-6 py-4 cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => handleSort('ward')}>Ward</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {paginatedData.length > 0 ? paginatedData.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-4 text-sm text-slate-600">{item.ward}</td>
            ...
            <form onSubmit={handleSave} className="p-6">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input required type="text" className="w-full px-4 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none" value={formData.ward} onChange={e => setFormData({...formData, ward: e.target.value})} />
                </div>
                <div>
