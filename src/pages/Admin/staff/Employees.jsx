...
import {
  ...
  ChevronLeft, ChevronRight, Activity, Users, UserCheck, UserX, Briefcase
} from 'lucide-react';
...
const MOCK_DATA = Array.from({ length: 55 }, (_, i) => ({
  id: `EMP-${1000 + i}`,
  name: `Employee ${['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis'][i % 8]} ${['A.', 'B.', 'C.', 'D.'][i % 4]}`,
  role: ['HR Manager', 'IT Support', 'Janitor', 'Security Officer', 'Driver'][i % 5],
  department: ['Human Resources', 'IT', 'Maintenance', 'Security', 'Transport'][i % 5],
...
const Employees = () => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    if (selectedRecord) {
      ...
      setData([{ ...values, id: `EMP-${Math.floor(Math.random() * 9000) + 1000}`, isDeleted: false, createdBy: 'Admin User', createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
    setSubmitting(false);
  };
  ...
  const columns = [
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('id')}>Employee ID</div>,
      accessor: 'id',
      render: (employee) => <span className="font-medium text-slate-900">{employee.id}</span>
    },
    {
    ...
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('role')}>Role & Dept</div>,
      accessor: 'role',
      render: (employee) => (
    ...
    },
    {
      ...
      render: (employee) => (
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-slate-400 uppercase">By {employee.createdBy}</span>
        </div>
      )
    },
    {
      ...
      render: (employee) => (
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${employee.status === 'Active' ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
            }`}
        >
          <div className={`w-1.5 h-1.5 rounded-full ${employee.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-400'}`}></div>
          {employee.status}
        </button>
      )
    },
    {
      ...
      render: (employee) => (
        <div className="flex items-center gap-2">
          <button onClick={() => openView(employee)} className="p-1.5 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" title="View Details">
            <Eye className="w-4 h-4" />
          </button>
          <button onClick={() => openEdit(employee)} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit">
            <Edit2 className="w-4 h-4" />
          </button>
          <button onClick={() => handleSoftDelete(employee.id)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete">
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Employees Management</h1>
          <p className="text-slate-500 text-sm mt-1">Manage general staff, administration, maintenance, and support personnel.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <button onClick={openCreate} className="w-full sm:w-auto flex justify-center items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium shadow-sm shadow-primary-600/20">
            <Plus className="w-4 h-4" /> <span className="whitespace-nowrap">Add Employee</span>
          </button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <StatCard title="Total Employees" value={data.filter(d => !d.isDeleted).length} icon={Users} colorTheme="blue" />
      ...
      <Modal
        ...
        title={selectedRecord ? 'Edit Employee' : 'Add New Employee'}
      >
        <Formik
          initialValues={selectedRecord || { name: '', role: '', department: '', email: '', phone: '', status: 'Active' }}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Field
                      as="select"
                      name="role"
                      className={`w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none ${errors.role && touched.role ? 'border-red-500' : 'border-slate-200'}`}
                    >
          ...
          <div>
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
                <div className="w-16 h-16 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 text-xl font-bold">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-600 font-medium">{selectedRecord.role} - {selectedRecord.department}</p>
                </div>
              </div>
