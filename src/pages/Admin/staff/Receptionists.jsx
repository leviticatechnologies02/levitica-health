...
const MOCK_DATA = Array.from({ length: 45 }, (_, i) => ({
  id: `REC-${1000 + i}`,
  name: `Receptionist ${['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis'][i % 8]} ${['A.', 'B.', 'C.', 'D.'][i % 4]}`,
  shift: ['Morning', 'Evening', 'Night'][i % 3],
  desk: ['Main Lobby', 'Emergency ER', 'Outpatient Dept', 'Billing', 'Admissions'][i % 5],
  email: `reception${i}@hospital.com`,
...
const ReceptionistSchema = Yup.object().shape({
  ...
  desk: Yup.string().required('Desk assignment is required'),
...
const Receptionists = () => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    if (selectedRecord) {
      ...
      setData([{ ...values, id: `REC-${Math.floor(Math.random() * 9000) + 1000}`, isDeleted: false, createdBy: 'Admin User', createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
    setSubmitting(false);
  };
  ...
  const filteredData = useMemo(() => {
    return data
      ...
      .filter(item => 
        ...
        item.desk.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const columns = [
    {
      ...
      accessor: 'id',
      render: (receptionist) => <span className="font-medium text-slate-900">{receptionist.id}</span>
    },
    {
      ...
      render: (receptionist) => (
        <div className="flex flex-col">
          <span className="text-sm font-medium text-slate-900">{receptionist.name}</span>
          <span className="text-xs text-slate-500">{receptionist.email}</span>
        </div>
      )
    },
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('shift')}>Shift & Desk</div>,
      accessor: 'shift',
      render: (receptionist) => (
        <div className="flex flex-col gap-1">
          <span className="inline-flex w-fit items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700">
            {receptionist.shift}
          </span>
          <span className="text-xs text-slate-500">{receptionist.desk}</span>
        </div>
      )
    },
    {
      ...
      render: (receptionist) => (
        <div className="flex flex-col">
          <span>{receptionist.createdAt}</span>
          <span className="text-[10px] text-slate-400 uppercase">By {receptionist.createdBy}</span>
        </div>
      )
    },
    {
      ...
      render: (receptionist) => (
        <button 
          ...
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${
            receptionist.status === 'Active' ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
          }`}
        >
          <div className={`w-1.5 h-1.5 rounded-full ${receptionist.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-400'}`}></div>
          {receptionist.status}
        </button>
      )
    },
    {
      ...
      render: (receptionist) => (
        <div className="flex items-center gap-2">
          <button onClick={() => openView(receptionist)} className="p-1.5 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" title="View Details">
            <Eye className="w-4 h-4" />
          </button>
          <button onClick={() => openEdit(receptionist)} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit">
            <Edit2 className="w-4 h-4" />
          </button>
          <button onClick={() => handleSoftDelete(receptionist.id)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete">
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Receptionists Management</h1>
          <p className="text-slate-500 text-sm mt-1">Manage hospital front desk staff, shift allocations, and desk assignments.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <button onClick={openCreate} className="w-full sm:w-auto flex justify-center items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium shadow-sm shadow-primary-600/20">
            <Plus className="w-4 h-4" /> <span className="whitespace-nowrap">Add Receptionist</span>
          </button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <StatCard title="Total Receptionists" value={data.filter(d => !d.isDeleted).length} icon={Users} colorTheme="blue" />
      ...
      <div className="bg-white p-4 rounded-t-xl border border-slate-100 border-b-0 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
        <div className="relative w-full sm:w-80">
          ...
          <input 
            type="text" 
            placeholder="Search by ID, Name, Shift or Desk..." 
      ...
      <Modal
        ...
        title={selectedRecord ? 'Edit Receptionist' : 'Add New Receptionist'}
      >
        <Formik
          initialValues={selectedRecord || { name: '', shift: '', desk: '', email: '', phone: '', status: 'Active' }}
            ...
            <Form className="p-6">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Desk Assignment</label>
                    <Field 
                      as="select" 
                      name="desk"
                      className={`w-full px-4 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none ${errors.desk && touched.desk ? 'border-red-500' : 'border-slate-200'}`}
                    >
                    ...
                    <ErrorMessage name="desk" component="div" className="text-red-500 text-xs mt-1" />
                  </div>
                </div>
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-600 font-medium">{selectedRecord.desk} - {selectedRecord.shift} Shift</p>
                </div>
              </div>
