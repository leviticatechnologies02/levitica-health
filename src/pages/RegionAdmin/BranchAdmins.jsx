...
const MOCK_ADMINS = Array.from({ length: 45 }, (_, i) => ({
  id: `BADM-${1000 + i}`,
  name: `Branch Admin ${['Smith', 'Johnson', 'Williams', 'Brown', 'Jones'][i % 5]}`,
  ...
  email: `branchadmin${i}@hospital.com`,
...
const BranchAdmins = () => {
  ...
  const handleExport = (format) => {
    alert(`Exporting branch admins as ${format.toUpperCase()}...`);
  };
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    if (selectedRecord) {
      setData(data.map(item => item.id === selectedRecord.id ? { ...values, id: item.id, branchName: item.branchName, createdAt: item.createdAt } : item));
    } else {
      setData([{ ...values, id: `BADM-${Math.floor(Math.random() * 9000) + 1000}`, branchName: 'New Assigned Branch', isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
    setSubmitting(false);
  };
  ...
  const filteredData = useMemo(() => {
    return data
      ...
      .filter(item =>
        ...
        item.branchId.toLowerCase().includes(search.toLowerCase())
      )
      .sort((a, b) => {
  ...
  const columns = [
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('id')}>Admin ID</div>,
      accessor: 'id',
      render: (admin) => <span className="font-medium text-slate-900">{admin.id}</span>
    },
    {
      ...
      render: (admin) => (
        <div className="flex flex-col">
          <span className="text-sm font-semibold text-slate-900">{admin.name}</span>
          <span className="text-xs text-slate-500">{admin.email} | {admin.phone}</span>
        </div>
      )
    },
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('branchName')}>Associated Branch</div>,
      accessor: 'branchName',
      render: (admin) => (
        <div className="flex items-center gap-1.5 cursor-pointer hover:text-primary-600 group" onClick={() => navigate('/regionAdmin/branches')}>
          <span className="text-sm font-medium bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md">{admin.branchName}</span>
        </div>
      )
    },
    {
      ...
      render: (admin) => (
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${admin.status === 'Active' ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
        >
          <div className={`w-1.5 h-1.5 rounded-full ${admin.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-400'}`}></div>
          {admin.status}
        </button>
      )
    },
    {
      ...
      render: (admin) => (
        <div className="flex items-center gap-2">
          <button onClick={() => openView(admin)} className="p-1.5 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" title="View Details">
            <Eye className="w-4 h-4" />
          </button>
          <button onClick={() => openEdit(admin)} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit">
            <Edit2 className="w-4 h-4" />
          </button>
          <button onClick={() => handleSoftDelete(admin.id)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete">
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Branch Admins</h1>
          {filterBranchId ? (
            <div className="flex items-center gap-2 mt-2">
              <span className="text-sm font-medium bg-primary-50 text-primary-600 px-2 py-1 rounded-md">Filtered by Branch: {filterBranchId}</span>
              <button onClick={() => navigate('/regionAdmin/branch-admins')} className="text-xs text-slate-500 hover:text-slate-900 underline">Clear Filter</button>
            </div>
          ) : (
            <p className="text-slate-500 text-sm mt-1">Manage administrators assigned to specific branches.</p>
          )}
        </div>
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <button onClick={openCreate} className="flex-1 sm:flex-none flex justify-center items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium shadow-sm shadow-primary-600/20">
            <UserCheck className="w-4 h-4" /> <span className="whitespace-nowrap">Add Admin</span>
          </button>
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <StatCard title="Total Admins" value={data.filter(d => !d.isDeleted).length} icon={Users} colorTheme="blue" />
        <StatCard title="Active Admins" value={data.filter(d => !d.isDeleted && d.status === 'Active').length} icon={UserCheck} colorTheme="green" />
        <StatCard title="Inactive Admins" value={data.filter(d => !d.isDeleted && d.status === 'Inactive').length} icon={ShieldAlert} colorTheme="red" />
      </div>
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={selectedRecord ? 'Edit Admin' : 'Add Branch Admin'} icon={UserCheck}>
        <Formik
          initialValues={selectedRecord || { name: '', branchId: filterBranchId || '', email: '', phone: '', status: 'Active' }}
            ...
            <Form className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <Field name="name" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all text-sm" placeholder="Enter admin name" />
                ...
                <div className="sm:col-span-2">
                  ...
                  <Field name="branchId" className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all text-sm" placeholder="e.g. BRN-1001" />
              ...
              <div className="flex justify-end gap-3 pt-4 mt-6 border-t border-slate-100">
                ...
                <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm font-medium shadow-sm shadow-primary-600/20 disabled:opacity-50 flex items-center gap-2">
                  {selectedRecord ? 'Save Changes' : 'Add Admin'}
                </button>
              </div>
            </Form>
          )}
      ...
      <Modal isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Branch Admin Details" icon={Eye}>
        {selectedRecord && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <p className="text-xs text-slate-500 mb-1">Associated Branch</p>
                <p className="text-sm font-medium text-primary-600 cursor-pointer hover:underline" onClick={() => { setIsViewOpen(false); navigate('/regionAdmin/branches'); }}>
