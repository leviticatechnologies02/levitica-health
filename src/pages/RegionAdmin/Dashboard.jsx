...
const Dashboard = () => {
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Region Overview</h1>
        <p className="text-slate-500 text-sm mt-1">Welcome back. Here's what's happening across your region today.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Groups" value="15" icon={Building2} colorTheme="blue" />
        <StatCard title="Active Branches" value="42" icon={ActivitySquare} colorTheme="green" />
        <StatCard title="Registered Patients" value="12,450" icon={UsersRound} colorTheme="purple" />
        <StatCard title="Region Admins" value="45" icon={Users} colorTheme="orange" />
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Patient admissions vs overall regional revenue</p>
            </div>
            <button onClick={() => navigate('/regionAdmin/reports')} className="text-sm font-medium text-primary-600 hover:text-primary-700 flex items-center gap-1">
