...
const MOCK_ADMINS = Array.from({ length: 25 }, (_, i) => ({
  id: `ADM-${1000 + i}`,
  name: `Admin ${['Smith', 'Johnson', 'Williams', 'Brown', 'Jones'][i % 5]}`,
  groupId: `GRP-${1000 + (i % 15)}`,
  groupName: `${['Apollo', 'Fortis', 'Max', 'Medanta', 'Manipal'][i % 5]} Healthcare Group`,
  email: `admin${i}@group.com`,
...
const GroupAdmins = () => {
  ...
  const handleExport = (format) => {
    alert(`Exporting group admins as ${format.toUpperCase()}...`);
  };
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    if (selectedRecord) {
      setData(data.map(item => item.id === selectedRecord.id ? { ...values, id: item.id, groupName: item.groupName, createdAt: item.createdAt } : item));
    } else {
      setData([{ ...values, id: `ADM-${Math.floor(Math.random() * 9000) + 1000}`, groupName: 'New Group Admin', isDeleted: false, createdAt: new Date().toISOString().split('T')[0] }, ...data]);
    }
    setIsFormOpen(false);
    setSubmitting(false);
  };
  ...
  const columns = [
    ...
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('name')}>Admin Name & Contact</div>,
      accessor: 'name',
      render: (admin) => (
    ...
    {
      header: <div className="cursor-pointer hover:text-primary-600 transition-colors" onClick={() => handleSort('groupName')}>Associated Group</div>,
      accessor: 'groupName',
      render: (admin) => (
        <div className="flex items-center gap-1.5 cursor-pointer hover:text-primary-600 group" onClick={() => navigate('/regionAdmin/groups')}>
          <span className="text-sm font-medium bg-primary-50 text-primary-700 px-2 py-1 rounded-md">{admin.groupName}</span>
        </div>
      )
    },
    {
  ...
  return (
    <div className="animate-in fade-in zoom-in-95 duration-500 pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Group Admins</h1>
          {filterGroupId ? (
            <div className="flex items-center gap-2 mt-2">
              ...
              <button onClick={() => navigate('/regionAdmin/group-admins')} className="text-xs text-slate-500 hover:text-slate-900 underline">Clear Filter</button>
            </div>
          ) : (
            <p className="text-slate-500 text-sm mt-1">Manage administrators for all organizational groups.</p>
          )}
        </div>
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={selectedRecord ? 'Edit Admin' : 'Add New Admin'} icon={UserCheck}>
        <Formik
          ...
          {({ isSubmitting }) => (
            <Form className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-slate-700 mb-1">Associated Group (ID) *</label>
      ...
      <Modal isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} title="Admin Details" icon={Eye}>
        {selectedRecord && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <p className="text-xs text-slate-500 mb-1">Associated Group</p>
                <div className="flex items-center gap-2">
                  <p className="text-sm font-medium text-slate-900">{selectedRecord.groupName} ({selectedRecord.groupId})</p>
                </div>
              </div>
