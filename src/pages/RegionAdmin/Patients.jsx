...
const Patients = () => {
  ...
  }, [data, search, statusFilter, sortConfig, filterBranchId, filterGroupId]);
  ...
  const columns = [
    ...
    },
    {
      ...
      accessor: 'branchName',
      render: (patient) => (
        <div className="flex flex-col cursor-pointer group" onClick={() => navigate('/regionAdmin/branches')}>
          ...
          {(filterBranchId || filterGroupId) ? (
            <div className="flex flex-wrap items-center gap-2 mt-2">
              <span className="text-sm font-medium bg-primary-50 text-primary-600 px-2 py-1 rounded-md">
                Filtered by {filterBranchId ? `Branch: ${filterBranchId}` : `Group: ${filterGroupId}`}
              </span>
              <button onClick={() => navigate('/regionAdmin/patients')} className="text-xs text-slate-500 hover:text-slate-900 underline whitespace-nowrap">Clear Filter</button>
            </div>
          ) : (
